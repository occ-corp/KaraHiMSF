...
class DivisionsController < ApplicationController
  ...
  def index
    if params[:format]=="csv"
      filename = I18n.t('filename_affiliation_list')
      if request.env['HTTP_USER_AGENT'] and request.env['HTTP_USER_AGENT'].include?('Win')
        send_data Division.to_csv.tosjis, :type => 'text/csv', :filename => filename.tosjis
      else
        send_data Division.to_csv, :type => 'text/csv', :filename => filename
      end
