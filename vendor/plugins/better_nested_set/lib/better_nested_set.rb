...
#                           acts_as_nested_set_options[:parent_column].intern
      ...
      module InstanceMethods
        ...
        # Move whole tree.  From Kevin Triplett's better_nested_set_hack.rb file
        #
        # this function allows a node to be moved from one tree to another
        # tree_hash is a single element hash in the form {:tree_id => <new tree id>}
        # target and position are optional, if omitted they default to adding the
        # node to the new tree as a subtree with virtual root at the right side of the tree.
        # postion defaults to the right side of the tree
        def move_to_tree(tree_hash, target=nil, position=:right)
          tree_col_name = tree_hash.keys.first
          new_tree_id = tree_hash[tree_col_name]

          if self[tree_col_name] != new_tree_id
            # sanity checks: make sure not new record
            # TODO: these tests are duplicated from BNS code, not DRY
            ...
            # get the node position and nest spread in the current tree
            my_lft, my_rgt = self[left_col_name], self[right_col_name]
            my_diff = my_rgt - my_lft + 1
            maxright = base_set_class.maximum(right_col_name, :conditions => scope_condition)

            # now get the new left index (before we add node to new tree) default to 1 if tree not in database yet
            self[tree_col_name] = new_tree_id
            my_new_lft = base_set_class.maximum(right_col_name, :conditions => scope_condition).to_i + 1
            self.reload

            transaction do
              # reassign the node and children to the new tree, and orphan the node
              base_set_class.update_all("\
                #{tree_col_name} = CASE \
                  WHEN #{left_col_name} BETWEEN #{my_lft} AND #{my_rgt} THEN #{new_tree_id} \
                  ELSE #{tree_col_name} END, \
                #{parent_col_name} = CASE \
                  WHEN #{self.class.primary_key} = #{self[self.class.primary_key]} THEN NULL \
                  ELSE #{parent_col_name} END",
                                        scope_condition)

              # now close up the tree being vacated (do this after assigning nodes to the new tree)
              base_set_class.update_all("\
                #{left_col_name} = CASE \
                  WHEN #{left_col_name} BETWEEN #{my_rgt+1} AND #{maxright} THEN #{left_col_name} - #{my_diff} \
                  ELSE #{left_col_name} END, \
                #{right_col_name} = CASE \
                  WHEN #{right_col_name} BETWEEN #{my_rgt+1} AND #{maxright} THEN #{right_col_name} - #{my_diff} \
                  ELSE #{right_col_name} END",
                                        scope_condition)

              # now renumber the node and all its children
              indexes = []
              self.reload
              calc_numbers(my_new_lft, indexes)
              indexes.each do |i|
                base_set_class.update_all("#{left_col_name} = #{i[:lft]}, #{right_col_name} = #{i[:rgt]}", "#{self.class.primary_key} = #{i[:id]}")
              end
              move_to(target, position) if target
            end
          end
        end
